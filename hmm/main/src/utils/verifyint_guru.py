...
os.chdir('/home/aslr/SBHMM-HTK/SequentialClassification/main/projects/Kinect_3signs/')
...
def verification_cmd(macros_filepath, verification_list, label_file, results_filepath, hresults_filepath, insertion_penalty: int = 0, beam_threshold: int = 2000):
    ...
    HVite_str = (f'HVite -A -H $macros -m -S {verification_list} -i '
...
def guru_collect_videos(video_directory, macros_filepath, features_config_filepath, results_filepath, hresults_filepath, game_output_filepath, acceptance_threshold: int = 0.01):
    if os.path.exists(game_output_filepath): os.remove(game_output_filepath)
    print(glob.glob(video_directory))
    video_filepaths = sorted(glob.glob(os.path.join(video_directory, '*', '*.mkv')))
    
    video_filepaths.sort(key=os.path.getmtime)
    ...
    #     myVideo = '/'.join(video_filepath.split('/')[-2:]).split('.')[0]
    #     # print(myVideo)
    #     if myVideo in data and data[myVideo] >= 100: continue
    #     if myVideo in data:
    #         data[myVideo] += 1
    #     else:
    #         data[myVideo] = 0
    #     print(video_filepath.split('/')[-1], data[myVideo])
    #     if video_filepath in working_videos:
    #         print(video_filepath, values)
    #         values[data[myVideo]] += 1
    #         print(video_filepath, values)
    #         data[myVideo] += 100

    # print(data)
    # print((([k for k,v in data.items() if v == 101])))


    # print(values)
    # for i, video in enumerate(video_filepaths):
    #     if i == 0: continue
    #     pastVideo = '.'.join(video_filepaths[i - 1].split('.')[:-3])
    #     myVideo = '.'.join(video.split('.')[:-3])

    #     if pastVideo != myVideo: continue

    #     pastVideo = video_filepaths[i - 1].split('.')[-2]
    #     myVideo = video.split('.')[-2]
    #     if int(pastVideo) >= int(myVideo):
    #         print(pastVideo, myVideo)

    # for video_filepath in video_filepaths:
    #     # if '5/alligator_above_blue_wall.2.1627256795.mkv' not in video_filepath: continue
...
def verify():
    ...
    parser.add_argument('--macros_filepath', type=str, default='/home/aslr/SBHMM-HTK/SequentialClassification/main/projects/Kinect_3signs/models/2/hmm15/newMacros')
    ...
    parser.add_argument('--features_config_filepath', type=str, default='/home/aslr/SBHMM-HTK/SequentialClassification/main/projects/Kinect_3signs/configs/features.json')
    parser.add_argument('--results_filepath', type=str, default='/home/aslr/SBHMM-HTK/SequentialClassification/main/projects/Kinect_3signs/testing_verification/verification_results.mlf')
    parser.add_argument('--hresults_filepath', type=str, default='/home/aslr/SBHMM-HTK/SequentialClassification/main/projects/Kinect_3signs/testing_verification/verifcation_hresults.json')
    parser.add_argument('--game_output_filepath', type=str, default='/home/aslr/SBHMM-HTK/SequentialClassification/main/src/utils/recognizer_all_UI_4sign.txt')

#
    parser.add_argument('--video_directory', type=str, default='/home/aslr/Documents/recordings/3-sign')

    # parser.add_argument('--video_directory', type=str, default='/run/user/1000/gvfs/sftp:host=ccg-charizard.cc.gt.atl.ga.us,user=thad/media/thad/U32 Shadow/Gururaj_PUA/3-sign/')

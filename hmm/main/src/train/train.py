...
def train(
    ...
    wordList_file: str = 'data/wordList_letter',
    hBuild_wordList_file: str = 'data/wordList_word',
    grammar_file: str = 'data/grammar.txt',
    ...
    if not is_bigram:
        ...
        for hmm0_file in tqdm(hmm0_files):
            HRest_command = (f'HRest -A -i 60 -C configs/hrest.conf -v 0.1 -I '
                             f'{all_labels_file} -M models/{fold}hmm1 -S lists/{fold}train.data '
                             f'{hmm0_file} >> logs/{fold}train.log')
            os.system(HRest_command)
    
    print('HRest Complete')

    print('Running HERest Iteration: 0...')
    if is_bigram:
        HERest_command = (f'HERest -A -T 2 -d models/{fold}hmm0 -c 500.0 -v 0.0005 -I '
                        f'{all_labels_file} -M models/{fold}hmm1 -S lists/{fold}train.data -T '
                        f'1 {wordList_file} >> logs/{fold}train.log')
    else:
        HERest_command = (f'HERest -A -d models/{fold}hmm1 -c 500.0 -v 0.0005 -I '
                        f'{all_labels_file} -M models/{fold}hmm2 -S lists/{fold}train.data -T '
                        f'1 {wordList_file} >> logs/{fold}train.log')
    ...
    if is_triletter:
        ...
        HLEd_command = (f'HLEd -A -D -T 1 -n wordList_triletter -l \'*\' -i all_labels_triletter.mlf '
                        f'mktri.led {all_labels_file} ')
        os.system(HLEd_command)
        MHed_command = (f'julia mktrihed.jl {wordList_file} wordList_triletter mktri.hed')
        os.system(MHed_command)
        HHed_command = (f'HHEd -A -D -T 1 -H models/{fold}hmm2/newMacros -M models/{fold}hmm3 mktri.hed {wordList_file} ')
        os.system(HHed_command)
        
    if is_triletter:
        start = 3
    else:
        start = 1 if is_bigram else 2
    ...
    for i, n_iters in enumerate(train_iters):
        ...
        for iter_ in tqdm(range(start, n_iters)):
            ...
            if is_triletter:
                HERest_command = (f'HERest -A -c 500.0 -v 0.0005 -D -T 2 -H '
                ...
                HERest_command = (f'HERest -A -c 500.0 -v 0.0005 -A -H '
                            f'models/{fold}hmm{iter_}/newMacros -I {all_labels_file} -M '
                            f'models/{fold}hmm{iter_+1} -S lists/{fold}train.data -T 1 {wordList_file} '
                            f'>> logs/{fold}train.log')
            os.system(HERest_command)
    ...
    if is_bigram:
        cmd = f'HBuild -A -T 1 -n {grammar_file} {hBuild_wordList_file} wordNet.txt'
    else:
        cmd = f'HParse -A -T 1 {grammar_file} wordNet.txt'
    os.system(cmd)

...
def get_data_from_description(description):
    ...
    if is_valid_exists:
        ...
        subbed = re.sub(r"signEnd=(.*?)\)", r'"\1")', subbed)
    subbed = re.sub(r"attempt=(.*?)", r"\1", subbed)
    subbed = re.sub(r"\"\[\((.*?)\)\]\"", r"[(\1)]", subbed)
    subbed = re.sub(r"\\/", r"/", subbed)
    data = eval(subbed)
...
def extract_clip_from_video(
    ...
    if is_valid_exists:
        filename, video_start_time, sign_start_time, sign_end_time, is_valid, _ = recording
    else:
        filename, video_start_time, sign_start_time, sign_end_time, _ = recording
        is_valid = True
...
def process_file(args, pool, pbar, filename, results, signs, recording_count):
    ...
    if filename.endswith("-timestamps.jpg"):
        ...
        if os.path.exists(videopath):
            ...
            if "expectedFrames" in data:
                del data["expectedFrames"]
